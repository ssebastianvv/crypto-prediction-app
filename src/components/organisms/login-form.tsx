...
export function LoginForm() {
  const [user, setUser] = useState<User | null>(null); // Permite tanto null como un objeto User

  const handleGoogleLogin = async () => {
    const userData = await signInWithGoogle();
    if (userData) {
      setUser(userData); // Ahora es válido porque puede ser User | null
  ...
  return (
    <div className="space-y-6 mt-6 bg-grey">
      <Button
        ...
        className="w-full bg-[#FCD535] hover:bg-[#F0B90B] text-black font-medium py-2"
      >
      ...
      {user && (
        <div className="mt-4">
          ...
          <Button onClick={logout} className="w-full bg-red-500 hover:bg-red-600 text-white font-medium py-2">
            Cerrar sesión
          </Button>
        </div>
      )}
