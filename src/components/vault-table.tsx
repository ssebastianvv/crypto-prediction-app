...
export function VaultTable({ priceData }: { priceData: PriceData[] }) {
  return (
    <Table>
      <TableHeader>
        <TableRow>
      ...
      <TableBody>
        {vaults.map((vault) => {
          const currentPriceData = priceData.find((data) => data.symbol === vault.symbol);
          const currentPrice = currentPriceData ? `$${currentPriceData.price.toFixed(2)}` : vault.price;
          ...
          return (
            <TableRow key={vault.symbol}>
              ...
              <TableCell>{currentPrice}</TableCell> {/* Precio actualizado */}
              <TableCell>
